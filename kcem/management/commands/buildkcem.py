...
import pyprind, json, multiprocessing, pymongo, logging
...
class Command(BaseCommand):
    ...
    def export2Mongo(self):
        ...
        keywordPieces = [keywordList[i:i + multiprocessing.cpu_count()] for i in range(0, len(keywordList), multiprocessing.cpu_count())]
        ...
        def activateKCEM(keywordThreadList):
            ...
            for index, keyword in enumerate(keywordThreadList):
                ...
                ThreadResult.append(criteria('hybrid', result[:10], keyword))
                if index % 10000 == 0:
